...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT){
    ...
    int block_num = NDIRECT+ bn/256;
    int entry_num = bn%256;
    if((addr = ip->addrs[block_num]) == 0)
      ip->addrs[block_num] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    if((addr = a[entry_num]) == 0){
  ...
  //level2
  if(bn < Ndouble){
    int block_num = bn/256;
    if((addr = ip->addrs[12]) == 0)// if no such block
      ip->addrs[12] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    if((addr = a[block_num]) == 0){
